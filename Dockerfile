FROM otomi/tools:v1.4.26 as ci
...
RUN if [ "$SKIP_TESTS" = 'false' ]; then ln -s $APP_HOME/tests/fixtures env && npm test && npm run test:diff && rm env; fi
...
#-----------------------------
FROM otomi/tools:v1.4.26 as prod
