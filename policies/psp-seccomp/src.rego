...
violation[{"msg": msg, "details": {}}] {
  ...
  msg := sprintf("Seccomp profile is not allowed, pod: %v, container: %v, Allowed profiles: %v", [metadata.name, container.name, parameters.parameters(policyID).allowedProfiles])
}
input_wildcard_allowed(metadata) {
  parameters.parameters(policyID).allowedProfiles[_] == "*"
}
input_container_allowed(metadata, container) {
  not get_container_profile(metadata, container)
  metadata.annotations["seccomp.security.alpha.kubernetes.io/pod"] == parameters.parameters(policyID).allowedProfiles[_]
}
input_container_allowed(metadata, container) {
  profile := get_container_profile(metadata, container)
  profile == parameters.parameters(policyID).allowedProfiles[_]
}
