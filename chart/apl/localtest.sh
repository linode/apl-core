...
# Set port forwarding so otomi the 'otomi install' can push values to gitea
# k port-forward -n gitea svc/gitea-http 3000:3000

export IN_DOCKER=false
# OTOMI_DEV_APPLY_LABEL - In local test you can narrow down the helm releases that are going to be installed while performing otomi install
