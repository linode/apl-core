...
git clone -c http.sslVerify=false --depth 10 http://$GITEA_USERNAME:$GITEA_PASSWORD@$url{{ if .DestDir }} {{ .DestDir }}{{ end }}
{{- else }}
while ! curl -m 3 -s -o /dev/null https://$GITEA_USERNAME:$GITEA_PASSWORD@$url; do
  echo "Waiting for the repository to be available"
  sleep 5s
done
git clone --depth 10 http://$GITEA_USERNAME:$GITEA_PASSWORD@$url{{ if .DestDir }} {{ .DestDir }}{{ end }}
{{- end }}
{{- end }}