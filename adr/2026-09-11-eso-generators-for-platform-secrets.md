...
Platform-internal passwords have been generated by the CLI at bootstrap time. The `x-secret` annotations in `values-schema.yaml` were checked for template tags (`{{` and `}}`) and processed through a tool named Gucci.
...
- Common `ClusterGenerator` resources (`alpha-num-32`, `random-32`, `random-16`) are defined in `values/external-secrets/external-secrets-raw.gotmpl`. They wrap a `Password` resource for re-use across the cluster.
