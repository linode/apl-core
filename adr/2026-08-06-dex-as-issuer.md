...
- **Logout is differently shaped and must be verified.** Keycloak's logout URL is wired into four places (`helmfile.d/snippets/routes.gotmpl`, `values/team-ns/team-ns.gotmpl`, `values/kubernetes-gateways/kubernetes-gateways-raw.gotmpl`, and apl-console's `Logout.tsx`). Dex supports RP-initiated logout at `/logout` — honouring `id_token_hint`, `post_logout_redirect_uri` and `state`, revoking refresh tokens and clearing the session cookie — but only advertises `end_session_endpoint` in its discovery document when the sessions feature is enabled (`DEX_SESSIONS_ENABLED=true`). This behaviour is described in the Dex repository's `docs/enhancements/auth-sessions-2026-02-18.md`, which is a design document rather than evidence of what shipped in a given tag, so **availability must be confirmed against the Dex version pinned** before the four call sites are rewired. If it is unavailable, logout degrades to clearing local sessions only.
...
- Bad, because logout is conditional on the sessions feature being enabled and available in the pinned version, and must be verified rather than assumed.
