...
- Branch proliferation: one branch per version produces `releases/v1.4.0-rc.1`, `releases/v1.4.0-rc.2`, `releases/v1.4.0`, `releases/v1.4.1` as separate branches for a single minor cycle

## Considered Options

- One branch per version (e.g., `releases/v1.4.0-rc.1`, `releases/v1.4.0`, `releases/v1.4.1`)
- One branch per major.minor cycle (e.g., `releases/v1.4`)
...
- A fix committed to `releases/v1.4` is automatically present in all subsequent tags from that branch — no cherry-picks needed
...
- The branch name (`releases/v1.4`) does not encode the current patch level; the authoritative version is always `package.json`, not the branch name
...
- Bad, because a fix on `releases/v1.4.0-rc.1` must be cherry-picked to `releases/v1.4.0-rc.2`, then again to `releases/v1.4.0`, then to `releases/v1.4.1` — four manual operations for one fix
