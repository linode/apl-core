...
gcloud container clusters get-credentials $CLUSTER_NAME --region $GOOGLE_REGION --project $PROJECT

kubectl create clusterrolebinding cluster-admin-binding --clusterrole=cluster-admin --user=$(gcloud config get-value account)

# # ADDITIONAL FIREWALL RULES FOR PRIVATE CLUSTER:
# SOURCE=$(gcloud container clusters describe $CLUSTER_NAME --region $GOOGLE_REGION | grep clusterIpv4CidrBlock | cut -d ':' -f 2 | tr -d ' ')
