...
export const preservePvcStorageClassInRawValues = async (
  ...
  deps: PreservePvcStorageClassDeps = {
    readPvc: async (namespace: string, name: string) => {
      try {
    ...
    listPvcs: async (namespace: string, labelSelector: string) => {
      const pvcList = await k8s.core().listNamespacedPersistentVolumeClaim({ namespace, labelSelector })
  ...
  const giteaDbPvcs = await deps.listPvcs('gitea', 'cnpg.io/cluster=gitea-db')
  const giteaDbDataPvc = giteaDbPvcs.find((pvc) => pvc?.metadata?.labels?.['cnpg.io/pvcRole'] === 'PG_DATA')
  maybeSetRawValue(
    'databases.gitea.storageClass',
    giteaDbDataPvc?.spec?.storageClassName || giteaDbPvcs[0]?.spec?.storageClassName,
  )
  ...
  const harborDbPvcs = await deps.listPvcs('harbor', 'cnpg.io/cluster=harbor-otomi-db')
  const harborDbDataPvc = harborDbPvcs.find((pvc) => pvc?.metadata?.labels?.['cnpg.io/pvcRole'] === 'PG_DATA')
  maybeSetRawValue(
    'databases.harbor.storageClass',
    harborDbDataPvc?.spec?.storageClassName || harborDbPvcs[0]?.spec?.storageClassName,
  )

  const keycloakDbPvcs = await deps.listPvcs('keycloak', 'cnpg.io/cluster=keycloak-db')
  const keycloakDbDataPvc = keycloakDbPvcs.find((pvc) => pvc?.metadata?.labels?.['cnpg.io/pvcRole'] === 'PG_DATA')
  maybeSetRawValue(
    'databases.keycloak.storageClass',
    keycloakDbDataPvc?.spec?.storageClassName || keycloakDbPvcs[0]?.spec?.storageClassName,
  )
