...
describe('disableGitServerIfMigrated', () => {
  ...
  it('disables git-server and checks all 3 apps when all conditions are met', async () => {
    ...
    expect(deps.getArgoCdApp).toHaveBeenCalledWith('apl-operator-apl-operator')
    expect(deps.getArgoCdApp).toHaveBeenCalledWith('argocd-argocd')
    expect(deps.getArgoCdApp).toHaveBeenCalledWith('otomi-otomi-api')
  })
})
