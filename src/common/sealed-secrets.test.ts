...
describe('sealed-secrets', () => {
  ...
  describe('buildSecretToNamespaceMap', () => {
    ...
    it('should skip users path (managed individually in apl-users namespace)', async () => {
      ...
      expect(result).toHaveLength(1)
      expect(result.find((m) => m.secretName === 'users-secrets')).toBeUndefined()
    })
  ...
  describe('writeSealedSecretManifests', () => {
    it('should write each manifest to the correct directory', async () => {
      ...
      expect(deps.mkdir).toHaveBeenCalledWith('/test/env/manifests/namespaces/apl-secrets/sealedsecrets', {
        recursive: true,
      })
      expect(deps.writeFile).toHaveBeenCalledWith(
        '/test/env/manifests/namespaces/apl-secrets/sealedsecrets/harbor-secrets.yaml',
        'yaml-content',
      )
    })
  })
